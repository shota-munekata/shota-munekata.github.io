...
function categorizeNews(title, content) {
    ...
    const categories = {
        // 地域・行政ニュース（優先度高）
        regional: ['市長', '知事', '議会', '市議', '県議', '選挙', '市政', '県政', '行政', '自治体', '住民', '地域', '市民', '県民', '市役所', '県庁', '発表', '決定', '条例', '予算', '経済', 'ビジネス', '企業', '会社', '商売', '売上', '業績', '投資', '株価', '市場', '商工会', '産業', '製造', '工場', '開発', '建設', '不動産', '求人', '雇用', '就職', '病院', '医療', '健康', '医師', '看護', '治療', '診療', '検診', '予防', '感染', 'ワクチン', '薬', '患者', '介護', '福祉', '高齢者', '学校', '教育', '生徒', '学生', '児童', '授業', '入学', '卒業', '受験', '試験', '大学', '高校', '中学', '小学', '幼稚園', '保育園', '先生', '教師', '校長'],
...
function generateWeatherData(region) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        weatherData.push({
            date: i === 0 ? '今日' : i === 1 ? '明日' : `${date.getMonth() + 1}/${date.getDate()}`,
...
function handleRegionChange(e) {
    ...
    updateWeatherForecast(e.target.value);
    filterAndDisplayNews();
}
